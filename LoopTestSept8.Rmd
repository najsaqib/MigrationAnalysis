...
#remove(subgroup)
IndexOrgName <- OrgNameID %>% filter(ORGID15 == id) %>% select(ORGANIZATION15)
...
kable(get(id), caption = "Table 2: Organizational Engagement State", digits = 1, 
      col.names = c("","Count 2013", "Count 2015", "Net Gain/Loss", "Percentage Change"), 
      align = c("l","c","c","c","c"))
...
{
get(id) %>% filter(EngagementScore==variable) %>% select(NetGainOrLoss)
}
